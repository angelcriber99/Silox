...
# 1. Update PortfolioShareSnapshot
...
# 2. Update PortfolioSharePresentation
content = content.replace(
    "let snapshot: PortfolioShareSnapshot",
    "let portfolio: PortfolioResponse\n    let initialBalancesHidden: Bool"
)

# 3. Update PortfolioSharePreviewSheet
old_sheet = r'(?s)struct PortfolioSharePreviewSheet: View \{.*?        \}\n    \}\n\}'
...
content = re.sub(old_sheet, new_sheet, content, count=1)

# 4. Update PortfolioShareCard summary section
old_summary = r'(?s)private var summary: some View \{\n.*?        \}'
...
# 5. Update PortfolioShareCard movementSection
old_movement = r'(?s)private var movementSection: some View \{.*?        \}'
...
content = re.sub(old_movement, new_movement, content, count=1)

# 6. Update ShareMovementRow
